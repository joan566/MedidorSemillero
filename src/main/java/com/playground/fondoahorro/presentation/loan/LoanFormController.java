...
public class LoanFormController {
    ...
    @FXML
    private Label totalPreviewLabel;
    @FXML
    ...
    /** Reflects Loan's own interest arithmetic in the UI before saving — no new business rule, just a preview. */
    private void updatePreview() {
        ...
        if (amount.isEmpty() || amount.get().signum() <= 0 || interestBps.isEmpty() || interestBps.get() < 0) {
            ...
            totalPreviewLabel.setText("—");
            return;
        }
        Loan preview = Loan.create(0, Money.of(amount.get()), interestBps.get(), LocalDate.now(), null);
        interestPreviewLabel.setText(MoneyFormatter.format(preview.interestAmount()));
        totalPreviewLabel.setText(MoneyFormatter.format(preview.totalAmount()));
    }
